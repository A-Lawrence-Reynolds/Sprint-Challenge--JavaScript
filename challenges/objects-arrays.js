...
const universities = [];
sortedUniversities = graduates.map((school) => school.university).sort()
console.log("sortedUniversities", sortedUniversities);
...
const unisWithUni = [];
graduates.filter((school) => school.university == "Uni")
...
lowCaseAnimalNames = zooAnimals.map((low) => low.animal_name.toLowerCase());
...
lowPopulationAnimals = zooAnimals.filter((needMore) => needMore.population < 5.)
